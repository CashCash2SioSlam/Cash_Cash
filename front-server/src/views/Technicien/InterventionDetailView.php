...
<div class="intervention-details bg-gray-100 p-6 ml-10 mr-10 mt-10 rounded-lg">
    <form method="post">
        <div class="grid grid-cols-2 gap-4">
            <h2 class="text-xl font-bold">Détails de l'intervention : <?php echo $interventionsTechnicien['NuméroIntervention']; ?></h2>
        ...
        <div class="grid grid-cols-2 gap-4">
                ...
                <select id="Matricule" name="Matricule" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
                    <?php foreach ($interventionsTechnicien as $interventionsTechnicien): ?>
                        <option value="<?php echo $interventionsTechnicien['Matricule']; ?>" <?php echo ($interventionsTechnicien['Matricule'] == $interventionsTechnicien['Matricule']) ? 'selected' : ''; ?>>
                            <?php echo $interventionsTechnicien['Matricule'] . ' - ' . $interventionsTechnicien['Nom'] . ' ' . $interventionsTechnicien['Prénom']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-1">
                ...
                <input type="text" id="NuméroClient" name="NuméroClient" value="<?php echo $interventionsTechnicien['NuméroClient']; ?>" placeholder="Exemple : 1" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div class="mb-1">
                <label for="TempsPassé" class="block text-sm font-medium text-gray-700">Temps passé :</label>
                <input type="Hour" id="TempsPassé" name="TempsPassé" value="<?php echo $interventionsTechnicien['TempsPassé']; ?>" placeholder="Exemple : 1" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="mb-1">
