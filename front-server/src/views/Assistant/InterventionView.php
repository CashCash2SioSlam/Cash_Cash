      ...
      <?php if (!empty($interventions)): ?>
        <?php foreach ($interventions as $intervention): ?>
          <div class="bg-gray-100 rounded-xl px-3 py-2 mt-2 ml-10 flex justify-between">
            <div>
              <?= $intervention['DateVisite'] ?> - <?= $intervention['HeureVisite'] ?> - <?= $intervention['Matricule'] ?> - <?= $intervention['NumeroClient'] ?><a href="../../../../../../Cash_Cash/back-server/src/PDF/AFI.php?intervention_id=<?= $intervention['NumeroIntervention'] ?>" target="_blank"><i class="fa-solid fa-file-arrow-down"></i></a>
            </div>
            <div class="flex">
              <form method="get" action="index.php" class="mr-2">
                  <input type="hidden" name="page" value="InterventionDetailView">
                  <input type="hidden" name="intervention_id" value="<?= $intervention['NumeroIntervention'] ?>">
