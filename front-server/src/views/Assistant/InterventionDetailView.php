...
<div class="intervention-details bg-gray-100 p-6 ml-10 mr-10 mt-10 rounded-lg">
    <form method="post">
        <div class="grid grid-cols-2 gap-4">
            <h2 class="text-xl font-bold">Détails de l'intervention : <?php echo $intervention['NumeroIntervention']; ?></h2>
                ...
                <select id="Matricule" name="Matricule" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
                    <?php foreach ($technicians as $technician): ?>
                        <option value="<?php echo $technician['Matricule']; ?>" <?php echo ($intervention['Matricule'] == $technician['Matricule']) ? 'selected' : ''; ?>>
                            <?php echo $technician['Matricule']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-1">
                <label for="NumeroClient" class="block text-sm font-medium text-gray-700">Numéro du client :</label>
                <input type="text" id="NumeroClient" name="NumeroClient" value="<?php echo $intervention['NumeroClient']; ?>" placeholder="Exemple : 1" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div class="mb-1">
                <label for="TempsPasse" class="block text-sm font-medium text-gray-700">Temps passé :</label>
                <input type="Hour" id="TempsPasse" name="TempsPasse" value="<?php echo $intervention['TempsPasse']; ?>" placeholder="Exemple : 1" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-1 py-1 mt-2 shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            <div class="mb-1">
